...
class ExportService:
        ...
        for i, (m_name, col) in enumerate(metrics, 2):
            ...
            if m_name == "Geometric Monthly Return":
                ...
                ws.cell(
                    row=i,
                    column=13,
                    value=f"=AVERAGE({col}3:{col}{last_row})"
                    if "Return" in m_name or "Premium" in m_name
                    else f"=VAR.P({col}3:{col}{last_row})",
                )
            ...
            if m_name == "Geometric Monthly Return":
                ...
                ws.cell(
                    row=i,
                    column=14,
                    value=f"=AVERAGE({col}{r5}:{col}{last_row})"
                    if "Return" in m_name or "Premium" in m_name
                    else f"=VAR.P({col}{r5}:{col}{last_row})",
                )
            ...
            if m_name == "Geometric Monthly Return":
                ...
                ws.cell(
                    row=i,
                    column=15,
                    value=f"=AVERAGE({col}{r3}:{col}{last_row})"
                    if "Return" in m_name or "Premium" in m_name
                    else f"=VAR.P({col}{r3}:{col}{last_row})",
                )
        ...
        for i, d in enumerate(dates, 2):
            ...
            if i > 2:
                for j, ticker in enumerate(tickers, 2):
                    ws.cell(
                        row=i, column=j, value=f"={get_column_letter(j)}{i - 1}*(1+'{ticker}'!C{i})"
                    )
                ...
                ws.cell(
                    row=i,
                    column=spy_col,
                    value=f"={get_column_letter(spy_col)}{i - 1}*(1+'S&P 500'!C{i})",
                )
                ws.cell(
                    row=i,
                    column=rf_col,
                    value=f"={get_column_letter(rf_col)}{i - 1}*(1+'Fred Graph'!C{i + 4})",
                )
    ...
    def _add_valuation_sheet(self, wb, valuation):
        ...
        for v in valuation.per_ticker:
            ws.cell(
                row=row, column=1, value=f"{v.ticker} Fundamental Valuation"
            ).font = self.title_font
            row += 2
    ...
    def _add_portfolio_org_sheet(self, wb, optimize):
        ...
        ws.cell(
            row=row + 2, column=2, value=optimize.complete.weight_risk_free
        ).number_format = "0.00%"
